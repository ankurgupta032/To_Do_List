showtask();  //agr refreash krogeto task tb bhi dikhenge..
...
addtaskbtn.addEventListener("click", function(){
    ...
    if(addtaskinputval.trim()!=0){
        let webtask = localStorage.getItem("localtask"); //localtask localstorage ki array ka nam hai...
        if(webtask == null){
            taskObj = [];
        }
        else{
            taskObj = JSON.parse(webtask); //local storage mein jo save hota hai string ke form mein save hoti hai..ab yeh object bn gya
        }
        taskObj.push({'task_name':addtaskinputval, 'completeStatus':false});
        localStorage.setItem("localtask", JSON.stringify(taskObj)); //make it string.. 
        addtaskinput.value = '';
    }
    showtask();
})
